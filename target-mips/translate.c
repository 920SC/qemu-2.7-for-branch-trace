                                ...
                                int delayslot_size)
{
    ...
    /* code write by sc */
   // ctx->tb->trace[0]=ctx->pc;// record branch opcode;
   // ctx->tb->trace[1]=ctx->opcode;// record branch pc;
    /* end */
...
static void gen_branch(DisasContext *ctx, int insn_bytes)
{
    if (ctx->hflags & MIPS_HFLAG_BMASK) {
        ...
        case MIPS_HFLAG_B:
            ...
            ctx->tb->class=0;//record br class;

            if (proc_hflags & MIPS_HFLAG_BX) {
                tcg_gen_xori_i32(hflags, hflags, MIPS_HFLAG_M16);
            }
            gen_goto_tb(ctx, 0, ctx->btarget);
            ctx->tb->trace[2]=ctx->btarget;
            break;
        case MIPS_HFLAG_BL:
            /* blikely taken case */

            ctx->tb->class=4;//record .. ;
        ...
        case MIPS_HFLAG_BC:
...

            {
                ...
                gen_goto_tb(ctx, 0, ctx->btarget);
                ctx->tb->trace[2]=ctx->btarget;
                ctx->tb->trace[3]=ctx->pc+insn_bytes;
            }
            break;
        case MIPS_HFLAG_BR:
            /* unconditional branch to register */
              
            ctx->tb->class=2;//record ..;
            ctx->tb->trace[2]=0xffffffffffffffff;
